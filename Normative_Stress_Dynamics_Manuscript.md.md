...
Here, $\sigma(x) = rac{1}{1+e^{-x}}$ represents the standard logistic sigmoid function.
...
* **Fatigue Collapse:** When adaptation is too slow ($ta pprox 0$), the network cannot raise its baseline capacity fast enough to counter the burnout penalty. The system enters a permanent sub-capacity state characterized by continuous, unyielding cascading failure.
* **True Self-Organized Criticality:** When $ta$ is perfectly balanced against the degradation forces of $eta$ and $\mu$, the system flawlessly self-tunes, permanently resting on the razor's edge of the topological percolation boundary.
* **Chaotic Oscillation:** When adaptation is excessively reactive (large $ta$), the system violently overcorrects to minor stimuli, expanding and contracting structural capacity in highly chaotic, destructive organizational boom-and-bust cycles.
...
Within the NSD framework, Systemic Entropy ($H$) quantitatively measures the macroscopic disorder and the spatial diffusion pattern of stress across the network. By defining the instantaneous stress probability mass of any given node $i$ as $p_i(t) = rac{S_i(t)}{\sum S_j(t)}$ (computed strictly under the necessary boundary condition that total network stress $\sum S_i > 0$), systemic entropy is evaluated using the classical Shannon entropy formulation [9]:
...
Mathematically, this forced redistribution produces a sudden, violent acceleration in entropy production, denoted by the time derivative $rac{dH}{dt} \gg 0$. This asymptotic spike consistently and reliably precedes the emergence of a giant failing component in network percolation models. Thus, entropy acceleration serves as a highly reliable, computable early-warning signal for topological phase transitions. In the Enron corpus, this precise $rac{dH}{dt}$ spike was mathematically observable months before CEO Jeffrey Skilling's resignation and the subsequent public bankruptcy.
...
However, if the network is perturbed by a significant cascading event ($F_i > 0$), the burnout penalty violently drives $rac{d\mathcal{F}_{NSD}}{dt} < 0$, rapidly bleeding usable free energy from the system. The socio-technical system remains Lyapunov stable in this trivial regime only if the external exogenous load does not push the network across the underlying saddle-node bifurcation boundary faster than the structural recovery parameter $\mu$ can restore it. This formulation theoretically bridges macroscopic sociophysics with the sophisticated free-energy principle deeply observed in biological and neural systems [10]. Biological brains, which similarly inhabit highly complex, high-dimensional state spaces, systematically maintain their stability by relaxing neural dynamics toward states that explicitly minimize internally defined functionals, such as free energy and hierarchical prediction error.
...
This specific biological tuning mechanism is governed by mathematics that are indistinguishable from NSD's Equation IV ($rac{dC_i^*}{dt} = ta(A_0 - A(t))$). Just as catastrophic disruptions to HSP have been directly implicated in the profound development of extreme hyperexcitability and severe epileptogenesis following traumatic brain injury (TBI), the absolute breakdown of the homeostatic adaptation rate in the NSD framework directly leads to the uncontrolled organizational seizure formally known as fatigue collapse.
...
Let $T_{t+1} \in \{0, 1\}$ represent the binary topological state of the system at the next time interval, where $T=1$ indicates a localized collapse (volatility exceeding three standard deviations) and $T=0$ indicates metastability. We model the probability of collapse, $P(T_{t+1} = 1)$, as a function of the system's current Efficiency Ratio ($ta_t$) and Stability Margin ($\Sigma_t$):

$$P(T_{t+1} = 1) = rac{1}{1 + xp(-(eta_0 + eta_1 ta_t + eta_2 \Sigma_t))}$$

Where:

* $eta_0$ is the baseline intercept of systemic friction.
* $eta_1$ is the coefficient for $ta_t$ (Efficiency/Load Ratio). NSD predicts $eta_1 > 0$, meaning as stress consumes capacity, the probability of catastrophic failure increases.
* $eta_2$ is the coefficient for $\Sigma_t$ (Absolute Stability Margin). NSD predicts $eta_2 < 0$, meaning that deep reserves of uncoerced surplus and integration capacity actively suppress the likelihood of collapse.
...
* **Efficiency / Load Ratio ($ta_t$):** The institutional leverage ratio.
...
2. **Validation of Theorem 2 (Phase Transition Collapse):** The failure of the hubs was not an isolated event. Dictated by Axiom 3 (Externalization Necessity), the hubs forcefully propagated their Loss Given Default (LGD) to their surviving counterparties. Because the peripheral nodes were also operating in a hyper-efficient regime ($ta > 0.90$), they lacked the necessary "thermodynamic firewalls" (redundant Integration Capacity) to absorb the externalized load. This triggered a rapid, non-linear percolation cascade. As observed in the simulation output , the cumulative failure rate followed a violent S-curve, transitioning the network from localized hub failure to 100% macroscopic topological collapse in merely four event-time steps.

...
Calculate $rac{dS_i}{dt}$ by balancing exogenous load ($L$), natural dissipation ($\gamma$), and topological redistribution ($\sum F$).
...
Update capacity $C_i$ based on the recovery rate ($\mu$) and the burnout penalty ($eta$) incurred during active cascades.
...
Quantify the disorder and diffusion of stress using the probability mass $p_i(t) = rac{S_i(t)}{\sum S_j(t)}$:


$$H(t) = - \sum_{i=1}^{N} p_i(t) \ln(p_i(t))$$


A sharp spike in $rac{dH}{dt} \gg 0$ indicates that localized stress containment has failed and a global phase transition is imminent.

#### **II. Normative Free Energy ($\mathcal{F}_{NSD}$) Monitoring**

Evaluate the remaining structural resilience using $\mathcal{F}_{NSD}(t) = \sum C_i(t) - 	au H(t)$. The system is globally stable only if $rac{d\mathcal{F}_{NSD}}{dt} \le 0$.

#### **III. Self-Organized Criticality (SOC) Validation**

Validate the "SOC Fingerprint" by testing if systemic cascade sizes $s$ follow a power-law distribution: $P(s) \sim s^{-lpha}$.

---

### 4. Interpretation: The Hysteresis Trap

Once a node undergoes a **Saddle-Node Bifurcation** into the low-capacity "Burnout" attractor ($C_i 	o 0$), recovery is non-linear. Returning a collapsed network to a stable state requires an asymmetric, deep reduction in exogenous load ($ho \ll 1$) for an extended duration to allow the recovery rate ($\mu$) to overcome the hysteresis deficit.
